...
export const ProductComponent: React.FC<ProductComponentProps> = ({ data, handleModalOpen }) => {
	...
	const { id, comments, full_name, offers, min_price } = data;
	...
	return <section className='product-page flex flex-col lg:flex-row justify-between gap-1 xl:gap-x-6'>
		<div className='max-w-[900px] flex-1 pr-3 xl:pr-5'>
			<div className='flex flex-col md:flex-row items-center md:items-start md:border-b border-[#DEE2EB] pb-5'>
				...
				<div className='flex-1 md:ml-6 xl:ml-20'>
					...
					<div className='offers mt-7'>
						{offers.map(item => {
							return <div key={item.cdate} className='offers__item grid md:grid-cols-9 gap-4 mt-3 py-3.5 md:py-0 px-5 md:px-0 bg-white md:bg-transparent border md:border-0 rounded-full'>
								<div className='input flex flex-row md:col-span-2 relative cursor-pointer'>
									...
									<label htmlFor='cb1' className='flex ml-2.5 md:ml-7 flex-col justify-center text-sm font-medium select-none'>{item.in_stock} шт.</label>
								</div>
			...
			<div className='purchase-information grid justify-self-stretch mt-5 md:mt-10'>
				...
				<button className='delivery-calculation btn secondary mt-6 text-sm font-medium border border-black w-full md:w-72'>
				...
				<div className='buttons-buy md:justify-self-end mt-8 md:0'>
					...
					<button onClick={() => handleModalOpen('QuickOrder')} className='btn secondary uppercase mt-2.5 w-full md:w-72'>
						<span className='ml-2.5'>{t('quick order')}</span>
					</button>
					<button onClick={() => handleModalOpen('OnlineInstallment')} className='btn success uppercase mt-2.5 w-full md:w-72'>
